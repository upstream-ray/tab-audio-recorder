...
3. 点击浏览器工具栏里的扩展图标。
4. 点击“开始录制当前标签页”。
5. 录制过程中可以点击“暂停录制”，暂停后可以点击“继续录制”。
6. 录制过程中可以关闭弹窗，录音不会中断。
7. 结束时点击“停止录制”，此时只会生成待导出的录音文件，不会立刻下载。
8. 确认需要保存后，点击“导出录音文件”。
9. 浏览器会下载一个 `.webm` 文件，文件名类似：`网页标题_YYYY-MM-DD_HH-mm-ss.webm`。
...
- 暂停录制只会暂停写入录音文件，不会暂停网页播放，也不会静音标签页。
- 停止录制后，录音文件会暂存在浏览器内存中，直到点击“导出录音文件”。
- 如果网页播放器或浏览器标签页被静音，可能录不到声音。
- 很长的录音会在录制中和待导出期间暂存在浏览器内存中。多小时录制建议后续使用自动分段保存版本。
